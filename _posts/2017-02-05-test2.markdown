...
The script from https://github.com/StefanScherer/dotfiles/blob/master/bin/add-shadow

You might need to `brew install imagemagick` before using it.

![Before add shadow]({{ site.baseurl }}/assets/images/add-shadow-before.png)


![After add shadow]({{ site.baseurl }}/assets/images/add-shadow-after.png)
...
| Lex noation       | Lex noation     | Set Theory Symbols    | 概念 |
| ---               | ---             | ---                   | ---                |
| `\mid`            | \mid            | $$ \mid            $$ | |
| `\lbrace`         | \lbrace \rbrace | $$ \lbrace \rbrace $$ | 也能用`\{\}` |
| `\in`             | \in             | $$ a \in A         $$ | a属于A element of|
| `\notin`          | \notin          | $$ a \notin A      $$ | 不属于  |
| `\ni`             | \ni             | $$ \ni             $$ | 也能用`\owns`|
| `\varnothing`     | \varnothing     | $$ \varnothing     $$ | 空集 |
| `\subset`         | \subset         | $$ A \subset B     $$ | A是B的子集        |
| `\subseteq`       | \subseteq       | $$ A \subseteq B   $$ | A是B的子集（一般用这个）|
| `\subsetneq`      | \subsetneq      | $$ A \subsetneq B  $$ | A是B的真子集 proper subset |
| `\supset`         | \supset         | $$ B \supset A     $$ | B包含A  | 
| `\supseteq`       | \supseteq       | $$ B \supseteq A   $$ | B包含A（一般用这个）include|
| `\supsetneq`      | \supsetneq      | $$ B \supsetneq A  $$ | B真包含A |
| `\cap`            | \cap            | $$ A \cap B        $$ | A与B的交集，A交B Intersection |
| `\cup`            | \cup            | $$ A \cup B        $$ | A与B的并集，A并B Union|
